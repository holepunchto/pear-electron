  ...
  class API extends api {
    ...
    constructor(ipc, state, teardown, id) {
      ...
      const media = {
        status: {
          microphone: () =>
            ipc.getMediaAccessStatus({ id, media: 'microphone' }),
      ...
      class Found extends streamx.Readable {
        ...
        keep() {
          ...
          return ipc
            .find({ id: this.#id, stop: 'keep' })
            .finally(() => this.destroy())
        }

        activate() {
          if (this.destroyed)
            throw Error('Nothing to activate, already destroyed')
          return ipc
      ...
      class GuiCtrl extends EventEmitter {
        ...
        static get parent() {
          if (!api.COMPAT)
            console.warn(
              'Pear.Window.parent & Pear.View.parent are deprecated use ui.app.parent'
            )
        ...
        static get self() {
          if (!api.COMPAT)
            console.warn(
              'Pear.Window.self & Pear.View.self are deprecated use ui.app'
            )
        ...
        constructor(entry, at, options = at) {
          ...
          if (!entry)
            throw new Error(
              `No path provided, cannot open ${this.constructor[kGuiCtrl]}`
            )
          this.entry = entry
        ...
        minimize() {
          if (this.constructor[kGuiCtrl] === 'view')
            throw new Error('A View cannot be minimized')
          return ipc.minimize({ id: this.id })
        }

        maximize() {
          if (this.constructor[kGuiCtrl] === 'view')
            throw new Error('A View cannot be maximized')
          return ipc.maximize({ id: this.id })
        }
    ...
    get tray() {
      if (!this.constructor.COMPAT)
        console.warn(
          "Pear.tray is deprecated use require('pear-electron').app.tray"
        )
    ...
    get badge() {
      if (!this.constructor.COMPAT)
        console.warn(
          "Pear.badge is deprecated use require('pear-electron').app.badge"
        )
    ...
    get media() {
      if (!this.constructor.COMPAT)
        console.warn(
          "Pear.media is deprecated use require('pear-electron').media"
        )
    ...
    get Window() {
      if (!this.constructor.COMPAT)
        console.warn(
          "Pear.Window is deprecated use require('pear-electron').Window"
        )
    ...
    get View() {
      if (!this.constructor.COMPAT)
        console.warn(
          "Pear.View is deprecated use require('pear-electron').View"
        )
