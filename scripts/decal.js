...
const fonts = fs
  .readFileSync(path.join(pathname, 'fonts.css'), { encoding: 'utf8' })
  .split('\n')
const decal = fs
  .readFileSync(path.join(pathname, 'decal.html'), { encoding: 'utf8' })
  .split('\n')
...
const index = decal.findIndex((line) =>
  line.includes('<link rel="stylesheet" href="./fonts.css">')
)
