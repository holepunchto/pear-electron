...
class PearElectron {
  constructor() {
    if (!Pear.config.assets.ui?.path) {
      const info = Pear.config.options.pre
        ? { assets: Pear.config.assets }
        : {
            assets: Pear.config.assets,
            hint: 'set pre: pear-electron/pre to autoset assets.ui'
          }
      throw new ERR_INVALID_CONFIG(
        'pear.assets.ui must be defined for project',
        info
      )
    }
    if (!Pear.config.assets.ui?.name) {
      throw new ERR_INVALID_CONFIG(
        'pear.assets.ui.name must be defined for project',
        { assets: Pear.config.assets }
      )
    }
  ...
  async start(opts = {}) {
    this.bin = path.join(
      Pear.config.assets.ui.path,
      'by-arch',
      require.addon.host,
      bin(Pear.config.assets.ui.name)
    )
    ...
    if (key === null) {
      ...
      if (isPath) {
        link =
          pathToFileURL(path.join(dir, base.entrypoint || '/')) + search + hash
    ...
    if (args.appling) {
      ...
      if (fs.existsSync(applingApp) === false)
